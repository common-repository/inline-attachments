...
class Inline_attachments {
	...
	function Inline_attachments(){
		...
		if(is_admin()) {
			...
			add_action( 'wp_ajax_ia_bulk_delete', array(&$this, 'ia_bulk_delete_attachments') );
	...
	function ia_bulk_delete_attachments() {
		...
		// TODO (maybe): Check if same file is used by another Attachment

		$idsArray = explode(',', $ids);

		if( !count($idsArray) ) {
			echo 'Inline Attachments: No valid ids found';
			return;
		}

		foreach($idsArray as $attachment_id){
			wp_delete_attachment( $attachment_id, true );
			echo "Inline Attachments: Deleting attachment #{$attachment_id}.";
		}
		exit;
	...
	function add_media_screen_css(){ ?>
		<style type="text/css" media="screen">
			...
			#media-upload .menu_order {
				text-align: center;
				margin: 0 !important;
				padding: 7px 20px 0 12px !important;
			}
			
			#media-upload .menu_order .menu_order_input {
